  ...
  (let [start-stypes (subscribe [:start-types])]
    (if (utils/is-mutating-mode? edit-mode)
      [v-box
       ...
       :children
       [
        ;[box :width "150px" :child [label :class "control-label" :label ""]]
        [single-dropdown
         :width "150px"
...
(defn booking-type-ddl [edit-mode booking]
  (let [leave-types (subscribe [:sorted-leave-types-by-deduction])
        ...
        type-showing-error-icon? (reagent/atom (seq (get-in booking [:validation-errors :leave-type-id])))
        type-showing-tooltip? (reagent/atom false)]
    (if (utils/is-mutating-mode? edit-mode)
      [v-box
       :children
       [
        [box :width "50px" :child [label :class "control-label" :label "Type"]]
        [h-box
         :justify :start
         :children
         [
          ...
          (show-error type-error-message type-showing-error-icon? type-showing-tooltip?)
          ]
         ]
        ]]
...
(defn booking-header []
  [h-box
   :align :end
   :children [[:span {:class "li_calendar fs1"
...
(defn header-and-close-button [edit-mode booking]
  [h-box
   :justify :between
   :children
   [[booking-header edit-mode booking]
  ...
  (let [edit-mode (subscribe [:booking-edit-mode (:booking-id bkng)])
    ...
    [h-box
     ...
     [[v-box
       ...
       :children
       [
        [header-and-close-button @edit-mode booking]
